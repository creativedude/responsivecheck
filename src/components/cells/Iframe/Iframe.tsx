...
export default class Iframe extends React.Component<any, any> {
    ...
    styles() {
        const initzoomlevel = this.iframeBlock ? (this.iframeBlock.offsetWidth - 30) / this.props.Iwidth : 1;
            ...
            <>
                <div id={"foo"+this.state.id} className={classes} ref={this.setiframeBlockRef} >
                    <div className="framedetails">
                        <h3>{this.props.name}<small><span> {this.props.Pwidth}</span> X <span>{this.props.Pheight} </span><span>/ {this.props.pixratio} </span></small></h3>
